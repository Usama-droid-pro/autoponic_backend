...
exports.getTempReadings = async (req, res) => {
	...
	temp2.map((ele) => {
		const createdAtUtc = utcToZonedTime(ele.createdAt, "Asia/Karachi");
		const createdAtLocal = zonedTimeToUtc(createdAtUtc, "Asia/Karachi");

		ele.temp2 = format(createdAtLocal, "yyyy-MM-dd HH:mm:ss a");
		console.log(ele.temp2);
	});
...
exports.getMoistureValue = async (req, res) => {
	...
	tomorrow.setDate(today.getDate() + 1);
	const moisture = await moistureModel.find({ createdAt: { $gte: today, $lt: tomorrow } });
	const moisture2 = JSON.parse(JSON.stringify(moisture));
	...
	res.status(200).json({ moisture2 });
};

exports.getMoistureGraph = async (req, res) => {
	const today = new Date();
	today.setHours(0, 0, 0, 0); // Set time to the beginning of the day
	let tomorrow = new Date(today);
	...
	const result = await fetchThreeHourIntervals(moisture2);
	console.log(result);
...
async function fetchThreeHourIntervals(records) {
	...
	for (let i = 0; i < 24; i += 3) {
		...
		const intervalKey = `${intervalStart}:00 ${
			intervalStart < 12 ? "AM" : "PM"
		} - ${intervalEnd}:00 ${intervalEnd < 12 ? "AM" : "PM"}`;
		if (intervals[intervalStart]) {
